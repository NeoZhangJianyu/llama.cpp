...
typedef tile<16, 8, sycl::half2> tile_B_16;
typedef tile<16,  8, float> tile_C_KQ;
typedef tile<16, 16, float> tile_C_KQ_16;
typedef tile<16, 4, sycl::half2> tile_C_VKQ;
typedef tile<16, 8, sycl::half2> tile_C_VKQ_16;
...
// nwarps_max:     maximum number of warps per SYCL block, up to 8 warps in total can run per SM (given enough shared memory).
...
struct fattn_mma_f16_config< 64,  64> {
    ...
    static constexpr int get_nbatch_K2_device(int /*ncols*/) {
        return 32;
    }
    ...
    static constexpr int get_nbatch_V2_device(int /*ncols*/) {
        return 32;
    }
    ...
    static constexpr int get_nbatch_combine_device(int /*ncols*/) {
        return 32;
    }
};
...
struct fattn_mma_f16_config<256, 256> {
    ...
    static constexpr int get_nbatch_combine_device(int ncols) {
#if DPCT_COMPATIBILITY_TEMP == GGML_SYCL_CC_TURING
        return ncols <= 16 ? 128 : 64;
#else
        GGML_UNUSED(ncols);
        return 128;
#endif // __SYCL_ARCH__ == GGML_SYCL_CC_TURING
    }
};
...
static __dpct_inline__ void flash_attn_ext_f16_load_tile(const sycl::half2 * const __restrict__ KV,
    ...
    if (use_cp_async) {
        ...
        constexpr int h2_per_chunk   = 16 / sizeof(sycl::half2);
        const int chunks_per_row = D2 / h2_per_chunk;

        const unsigned int tile_KV_32 = ggml_sycl_cvta_generic_to_shared(tile_KV);
...
#pragma unroll
        ...
        ggml_sycl_unroll<3>{}(load);
    }
}

template <int ncols1, int nwarps, int nbatch_fa, bool use_cp_async>
static __dpct_inline__ void flash_attn_ext_f16_load_mask(const sycl::half2 * const __restrict__ mask_h2,
...
#pragma unroll
        for (int j0 = 0; j0 < ncols1; j0 += stride_j) {
            ...
            const int i = 4 * (item_ct1.get_local_id(2) % (nbatch_fa / 8));

            cp_async_cg_16<preload>(tile_mask_32 + j * (nbatch_fa * sizeof(sycl::half) + 16) + i * sizeof(sycl::half2),
                                    mask_h2 + j * stride_mask + i);
        }
        return;
    }
...
template <int  DKQ,
          ...
          bool last_iter>
/*
DPCT1110:19: The total declared local variable size in device function flash_attn_ext_f16_iter exceeds 128 bytes and may cause high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
static __dpct_inline__ void flash_attn_ext_f16_iter(const sycl::float2 * const __restrict__ Q_f2,
                                                    const sycl::half2 * const __restrict__ K_h2,
                                                    const sycl::half2 * const __restrict__ V_h2,
                                                    const sycl::half2 * const __restrict__ mask_h2,
                                                    sycl::float2 * const __restrict__ dstk,
                                                    sycl::float2 * const __restrict__ dstk_fixup,
                                                    const float scale,
                                                    const float slope,
                                                    ...
                                                    const int   stride_V,
                                                    const int   stride_mask,
                                                    sycl::half2 * const __restrict__ tile_Q,
                                                    sycl::half2 * const __restrict__ tile_K,
                                                    sycl::half2 * const __restrict__ tile_V,
                                                    sycl::half2 * const __restrict__ tile_mask,
    ...
    constexpr int np              = nwarps * (cols_per_warp/ncols2) / ncols1; // Number of parallel SYCL warps per Q column.
...
#pragma unroll
    for (int k0_start = 0; k0_start < DKQ/2; k0_start += nbatch_K2) {
        ...
        if (nstages <= 1) {
            ...
            /*
            DPCT1118:20: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
            */
            /*
...
#pragma unroll
            for (int l = 0; l < tile_C_KQ::ne; ++l) {
                KQ_C[i].x[l] = logit_softcap * sycl::tanh((float) KQ_C[i].x[l]);
            }
        }
    }
...
#pragma unroll
            for (int l = 0; l < tile_C_KQ::ne; ++l) {
                KQ_max_new[l % 2] = sycl::fmax((float) KQ_max_new[l % 2], (float) KQ_C[k].x[l]);
            }
        }
...
#pragma unroll
            for (int offset = 16; offset >= 4; offset >>= 1) {
                KQ_max_new[col] =
                    sycl::fmax((float) KQ_max_new[col],
...
#pragma unroll
            for (int l = 0; l < tile_C_KQ::ne; ++l) {
                KQ_C[k].x[l] = sycl::native::exp((float) (KQ_C[k].x[l] - KQ_max_new[l % 2]));
...
#pragma unroll
        for (int col = 0; col < cols_per_thread; ++col) {
            ...
            KQ_max_scale[col]       = sycl::native::exp(KQ_max_diff);
        ...
        if (ntiles == 1) {
            const sycl::half2 KQ_max_scale_h2 = make_half2(KQ_max_scale[0], KQ_max_scale[1]);
#pragma unroll
...
#pragma unroll
            for (int col = 0; col < cols_per_thread; ++col) {
                const sycl::half2 KQ_max_scale_h2 = make_half2(KQ_max_scale[col], KQ_max_scale[col]);
#pragma unroll
...
#pragma unroll
    for (int i0_stop = DV; i0_stop > 0; i0_stop -= 2*nbatch_V2) {
        ...
        const sycl::half2 * tile_V_i = i0_start < reusable_cutoff ? tile_V : tile_V + (i0_start - reusable_cutoff) / 2;
...
template <int  DKQ,
          ...
          bool is_fixup>
/*
DPCT1110:25: The total declared local variable size in device function flash_attn_ext_f16_process_tile exceeds 128 bytes and may cause high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
static __dpct_inline__ void flash_attn_ext_f16_process_tile(const sycl::float2 * const __restrict__ Q_f2,
                                                            const sycl::half2 * const __restrict__ K_h2,
                                                            const sycl::half2 * const __restrict__ V_h2,
                                                            const sycl::half2 * const __restrict__ mask_h2,
    ...
    auto          tile_Q = (sycl::half2 *) dpct_local;
    sycl::half2 * tile_K = c::Q_in_reg ? tile_Q : tile_Q + ncols * stride_tile_Q;
    sycl::half2 * tile_V = nstages > 1 ? tile_K + c::nbatch_fa * stride_tile_K : tile_K;
    sycl::half2 * tile_mask =
        nstages > 1 ? tile_V + c::nbatch_fa * stride_tile_V : tile_V + c::nbatch_fa * stride_tile_KV_max;
...
#pragma unroll
    ...
    */
    ...
    if (c::Q_in_reg) {
        const int j0 = (item_ct1.get_local_id(1) / np) * cols_per_warp;
...
#pragma unroll
    ...
    */
    ...
    if (nstages > 1 && nwarps*cols_per_warp > c::nbatch_fa) {
        /*
        DPCT1118:26: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
        */
        /*
...
#pragma unroll
    ...
    if (!is_fixup && (np == 1 || item_ct1.get_local_id(1) % np == 0) && sinks_f) {
        float KQ_max_scale[cols_per_thread];
#pragma unroll
        for (int col = 0; col < cols_per_thread; ++col) {
            ...
            const float KQ_max_new  = sycl::fmax((float) KQ_max[col], sink);
            ...
            *((uint32_t *) &KQ_max_scale[col]) *= KQ_max_diff >= SOFTMAX_FTZ_THRESHOLD;
            ...
            const float KQ_max_add = sycl::native::exp(sink - KQ_max_new);
...
#pragma unroll
    ...
    if constexpr (ntiles == 1) {
        ...
        */
        ...
        if (((!needs_fixup && !is_fixup) || np > 1) &&
            (ntiles == 4 || item_ct1.get_local_id(2) % 4 < cols_per_thread)) {
            // Use the 16 bytes of padding in each row to store the meta data: KQ max, KQ rowsum, KQ max scale.
            ((sycl::float2 *) tile_Q)[jc_cwm * (tile_stride / 2) + nbatch_combine / 2] = KQ_cmr;
        }
        ...
        */
        ...
        if (np == 1) {
            // No combination is needed, the meta data can be directly written from registers to VRAM.
            if (needs_fixup && (ntiles == 4 || item_ct1.get_local_id(2) % 4 < ntiles)) {
                sycl::float2 * dstk_fixup_meta = dstk_fixup + item_ct1.get_group(2) * ncols;
                dstk_fixup_meta[jc_cwm] = KQ_cmr;
            }
            if (is_fixup && (ntiles == 4 || item_ct1.get_local_id(2) % 4 < ntiles)) {
                sycl::float2 * dstk_fixup_meta =
                    dstk_fixup + (item_ct1.get_group_range(2) + item_ct1.get_group(2)) * ncols;
                dstk_fixup_meta[jc_cwm] = KQ_cmr;
            }
...
#pragma unroll
        ...
        */
        /*
        DPCT1065:64: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
        */
        item_ct1.barrier();
...
#pragma unroll
        ...
        /*
        DPCT1118:28: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
        */
        /*
...
#pragma unroll
        ...
        */
        /*
        DPCT1065:66: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
        */
        item_ct1.barrier();
...
#pragma unroll
        ...
        if (np > 1) {
            ...
            */
            /*
            DPCT1065:67: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
            */
            item_ct1.barrier();
        }
    }
...
#    if DPCT_COMPATIBILITY_TEMP == GGML_SYCL_CC_TURING
    ...
    const int stride_Q1   = nb01 / sizeof(sycl::float2);
    const int stride_Q2   = nb02 / sizeof(sycl::float2);
    const int stride_K    = nb11 / sizeof(sycl::half2);
    const int stride_mask = nb31 / sizeof(sycl::half2);

    const int stride_V = mla ? stride_K : nb21 / sizeof(sycl::half2);
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        const sycl::half2 *  mask_h2 = ncols2 == 1 && !mask ?
                                           nullptr :
                                           (const sycl::half2 *) (mask + nb33 * (sequence % ne33) + nb31 * jt * ncols1);
        sycl::float2 *       dstk    = ((sycl::float2 *) dst) + (sequence * ne01 * ne02 + head0) * (DV / 2);

        const sycl::half2 * V_h2 =
            mla ? K_h2 + (DKQ / 2 - DV / 2) : (const sycl::half2 *) (V + nb23 * sequence + nb22 * (head0 / gqa_ratio));
        ...
        if (KV_max) {
            kb0_stop_kernel = dpct::min(kb0_stop_kernel, KV_max[sequence * iter_j + jt] / c::nbatch_fa);
        }
...
void ggml_sycl_flash_attn_ext_mma_f16_case(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    const size_t nbytes_shared_KV_1stage = c::nbatch_fa * std::max(nbatch_K2 + 4, nbatch_V2 + 4) * sizeof(sycl::half2);
    const size_t nbytes_shared_KV_2stage = c::nbatch_fa * (nbatch_K2 + 4 + nbatch_V2 + 4) * sizeof(sycl::half2);
    const size_t nbytes_shared_Q         = ncols * (DKQ / 2 + 4) * sizeof(sycl::half2);
    const size_t nbytes_shared_mask      = ncols1 * (c::nbatch_fa / 2 + 4) * sizeof(sycl::half2);
    const size_t nbytes_shared_combine   = nwarps * cols_per_warp * (nbatch_combine + 4) * sizeof(sycl::half2);
...
#define DECL_FATTN_MMA_F16_CASE(DKQ, DV, ncols1, ncols2)                          \
    template void ggml_sycl_flash_attn_ext_mma_f16_case                           \
    <DKQ, DV, ncols1, ncols2>(ggml_backend_sycl_context & ctx, ggml_tensor * dst) \
